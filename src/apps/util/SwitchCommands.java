...
public class SwitchCommands {
    ...
    private static Logger log = LoggerFactory.getLogger(SwitchCommands.class.getSimpleName());
     ...
     * @param priority the priority of the rule; larger values are higher priority
     * @param matchCriteria the match criteria for the rule
     * @param instructions the actions to apply to packets matching the rule
     * @param hardTimeout the rule should be removed after hardTimeout seconds have elapsed since the rule was installed; if 0, then the rule will never be removed
     * @param idleTimeout the rules should be removed after idleTimeout seconds have elapsed since a packet last matched the rule; if 0, then the rule will never be removed due to a lack of matching packets
     * @param bufferId apply the newly installed rule to the packet buffered in this provided slot on the switch
     * @return true if the rule was sent to the switch, otherwise false
     */
    public static boolean installRule(IOFSwitch sw, byte table, short priority, OFMatch matchCriteria, List<OFInstruction> instructions, short hardTimeout, short idleTimeout, int bufferId) {
     ...
     * @return true if the rule was sent to the switch, otherwise false
     */
    public static boolean installRule(IOFSwitch sw, byte table, short priority, OFMatch matchCriteria, List<OFInstruction> instructions, short hardTimeout, short idleTimeout) {
    	return installRule(sw, table, priority, matchCriteria, instructions, hardTimeout, idleTimeout, OFPacketOut.BUFFER_ID_NONE);
    }
     ...
     * @return true if the rule was sent to the switch, otherwise false
     */
    public static boolean installRule(IOFSwitch sw, byte table, short priority, OFMatch matchCriteria, List<OFInstruction> instructions) {
    	return installRule(sw, table, priority, matchCriteria, instructions, NO_TIMEOUT, NO_TIMEOUT);
    }
     ...
     */
    public static boolean removeRules(IOFSwitch sw, byte table, OFMatch matchCriteria) {
	 ...
	 */
	public static boolean sendPacket(IOFSwitch outSw, short outPort, Ethernet eth) {
        ...
        pktOut.setLength((short)(OFPacketOut.MINIMUM_LENGTH + pktOut.getActionsLength() + packetData.length));
